...
export function Categories({ data = {} }) {
  const { title = '', titleAccent = '', description = '', items = [] } = data;
  ...
  const maxIndex = Math.max(0, items.length - 2); // last valid pair
  ...
  // Far background cards (very blurry, barely visible)
  if (currentIndex + 2 < items.length) slots.push({ index: currentIndex + 2, role: 'far-right-1' });
  if (currentIndex + 3 < items.length) slots.push({ index: currentIndex + 3, role: 'far-right-2' });

  const visibleSlots = slots.filter((slot) => items[slot.index]);
      ...
      <div className="max-w-[1600px] mx-auto relative z-10">
        ...
        >
          <h2 className="text-5xl sm:text-6xl text-white mb-4 drop-shadow-lg">
            {title} <span className="text-[#E89B3C] font-serif italic">{titleAccent}</span>
          </h2>
