...
function ProductColumn({ products, columnIndex, onAddToCart, onToggleSave, isSaved }) {
              ...
              <div className="relative h-[440px] overflow-hidden rounded-t-2xl">
                ...
                <div className="absolute top-4 right-4 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                  <button
                    ...
                    className="bg-white/95 backdrop-blur-sm p-3 rounded-full shadow-xl hover:bg-[#531323] hover:text-white transition-colors"
                  >
                    <Heart
                      className={`w-5 h-5 ${isSaved(product.id) ? 'fill-[#531323] text-[#531323]' : ''}`}
                    />
                  </button>
                  <button
                    type="button"
                    ...
                    className="bg-white/95 backdrop-blur-sm p-3 rounded-full shadow-xl hover:bg-[#531323] hover:text-white transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                  >
              ...
              <div className="relative h-[160px] px-6 pt-4 pb-6" style={{ background: 'linear-gradient(180deg, #FFF9F0 0%, #ffffff 100%)' }}>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <button
                    ...
                    onClick={() => onAddToCart(product)}
                    disabled={!product.inStock}
                    className="bg-[#531323] hover:bg-[#731830] text-white px-6 py-2.5 rounded-full transition-all duration-300 transform group-hover:scale-105 shadow-lg hover:shadow-xl font-medium disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none"
                  >
...
export function FeaturedCollection({ data = {} }) {
  ...
  const {
    ...
    (options) => catalog.products({ featured: true, limit, sort: 'featured' }, options),
    [limit],
  );

  // One fetch, split into the two scrolling columns the layout expects.
  const [columnOne, columnTwo] = useMemo(() => {
    const cards = (response?.data ?? []).map((product) => ({
      id: product.id,
      slug: product.slug,
      name: product.name,
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        >
          <h2 className="text-5xl sm:text-6xl text-white mb-4 drop-shadow-lg">
            <span className="text-[#E89B3C] font-serif italic">{titleAccent}</span> {title}
          </h2>
        ...
        {actionError ? (
          <p className="text-center text-[#E89B3C] text-sm mb-8" role="alert">{actionError}</p>
        ) : null}

        {/* Two Column Layout */}
        {loading ? (
          <InlineLoader className="mb-16" />
        ) : error ? (
          <ErrorState
            className="mb-16"
          ...
          <p className="text-center text-white/60 italic font-serif mb-16">
        ...
        >
          {ctaLabel ? (
            <Link
              to={ctaHref || '/shop'}
              className="inline-block bg-gradient-to-r from-[#531323] to-[#E89B3C] hover:from-[#731830] hover:to-[#D4823A] text-white px-16 py-5 text-lg rounded-full shadow-2xl hover:shadow-3xl transition-all duration-300 transform hover:scale-105"
            >
              {ctaLabel}
            </Link>
          ) : null}
